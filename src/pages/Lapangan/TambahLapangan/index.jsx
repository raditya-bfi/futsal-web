...
import AddLapanganFoto from './AddLapanganFoto'
import useCustom from './hooks'
import { AddFieldSchema, AddFieldSchemaOption } from './schema'
import useStyles from './style'

function TambahLapanganPage() {
  const { handler, refs, state } = useCustom()
  const classes = useStyles()

  return (
    <>
      <Helmet title='Tambah Lapangan'>
        <meta name='description' content='Tambah Lapangan | Futsal Gembira' />
      </Helmet>
      <Snackbar
      ...
      <Box className={classes.container}>
        <Box className={classes.pageTitle}>
          <Box className={classes.titleWrapper}>
            ...
            <Typography className={classes.title}>Tambah Lapangan</Typography>
          </Box>
        </Box>
        <Formik
                    ...
                    <Box className={classes.fieldSlider}>
                      ...
                      <Box className={classes.fieldSliderContainer}>
                        ...
                        <Box className={classes.fieldSliderWrapper}>
                          {state?.firstPhotos[0] &&
                          state?.firstPhotos[0]?.preview !== null &&
                          state?.firstPhotos[0]?.preview !== undefined ? (
                            <Box
                              className={classes.fieldSlidePhoto}
                              key='field-photo-preview-0'
                              sx={{ backgroundImage: `url(${state?.firstPhotos[0]?.preview})` }}
                            >
                              <Box className={classes.closeWrapper}>
                                <IconButton
                                  onClick={() => {
                                    handler.setFirstPhotos([])
                                  }}
                                >
                                  <CloseOutlined />
                                </IconButton>
                          ...
                          state?.secondPhotos[0]?.preview !== null &&
                          state?.secondPhotos[0]?.preview !== undefined ? (
                            <Box
                              className={classes.fieldSlidePhoto}
                              key='field-photo-preview-1'
                              sx={{ backgroundImage: `url(${state?.secondPhotos[0]?.preview})` }}
                            >
                              <Box className={classes.closeWrapper}>
                                <IconButton
                                  onClick={() => {
                                    handler.setSecondPhotos([])
                                  }}
                                >
                                  <CloseOutlined />
                                </IconButton>
...
export default TambahLapanganPage
